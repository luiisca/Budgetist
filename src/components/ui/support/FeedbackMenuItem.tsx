...
interface FeedbackMenuItemProps {
  onFeedbackItemSelect: () => void;
}

export default function FeedbackMenuItem({
  onFeedbackItemSelect,
}: FeedbackMenuItemProps) {
  const [rating, setRating] = useState<null | string>(null);
  const [comment, setComment] = useState("");
  const [disableSubmit, setDisableSubmit] = useState(true);

  const mutation = trpc.user.submitFeedback.useMutation({
    onSuccess: () => {
      setDisableSubmit(true);
      showToast("Thank you, feedback submitted", "success");
      onFeedbackItemSelect(); // close feedback menu
    },
  });
  ...
  return (
    <div className="w-full rounded-md border-gray-300 bg-white">
      <div className="w-full p-5">
        <p className="mb-1 text-neutral-500">Feedback</p>
        <p className="flex w-full py-2 text-sm font-medium text-gray-700">
        ...
        {mutation.isError && (
          <div className="mb-4 flex bg-red-100 p-4 text-sm text-red-700">
            ...
            <div className="ml-3 flex-grow">
              <p className="font-medium">Error sending feedback</p>
