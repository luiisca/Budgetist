  ...
  const errorMessages: { [key: string]: string } =
    {
      ...
      [ErrorCode.ThirdPartyIdentityProviderEnabled]: "Your account was created using an Identity Provider.",
      [ErrorCode.IncorrectProvider]: "Account already registered with another provider (Google, Facebook or Magic Link). Please try again using the one you registered your account with. Try to reset your browser if this problem persists.",
    }
  ...
  useEffect(() => {
        if (router.query?.error) {
            if (router.query?.error === "OAuthCallback" || router.query?.error === "OAuthAccountNotLinked") {
                ...
                setOAuthError(true);
                setErrorMessage(errorMessages[router.query?.error as string] || "Something went wrong.");
            }
            // Clean URL to clean error query
            router.push(`${WEBAPP_URL}/auth/login`, undefined, {shallow: true})
        }
  }, [errorMessages, router.query?.error]);
